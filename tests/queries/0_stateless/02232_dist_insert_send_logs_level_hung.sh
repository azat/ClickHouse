...
# it is easy to archive using materialized views.
...
#
# NOTE: using proper termination (via SIGINT, via clickhouse_client_timeout + clickhouse_test_wait_queries) is too long,
...
timeout 10s $CLICKHOUSE_CLIENT "${client_opts[@]}" -nm -q "
    drop table mv_02232;
    drop table in_02232;
