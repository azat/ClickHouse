    ...
    QueryPlanStepPtr analyzed_read_from_merge_tree)
{
    ...
    if (settings[Setting::allow_experimental_analyzer] && settings[Setting::parallel_replicas_local_plan] && !shard_num && !settings[Setting::parallel_replicas_two_phase_index_analysis])
    {
