...
| `clickhouse-X.Y` | Test is run only against a server of version X.Y or newer | Test creates a table that older servers accept but the current server refuses to load, which breaks the upgrade check |
